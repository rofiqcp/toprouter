<div align="center">
  <img src="../images/toprouter.png?1" alt="9Router Dashboard" width="800"/>
  ...
  **OpenClaw 的免费 AI 提供商。**
  ...
  [🚀 快速开始](#-quick-start) • [💡 特性](#-key-features) • [📖 设置](#-setup) • [🌐 网站](https://toprouter.com)
</div>
...
此仓库包是私有的（`toprouter-app`），因此源码/Docker 执行是预期的本地开发路径。
...
| 变量 | 默认值 | 描述 |
|----------|---------|-------------|
| `JWT_SECRET` | 自动生成（`~/.toprouter/jwt-secret`） | 仪表板认证 cookie 的 JWT 签名密钥（设置可在多实例间共享） |
| `INITIAL_PASSWORD | `123456` | 当没有保存的哈希时的首次登录密码 |
| `DATA_DIR` | `~/.toprouter` | 主应用数据库位置（`db.json`） |
...
| `CLOUD_URL` | `https://toprouter.com` | 服务器端云同步端点基础 URL |
| `NEXT_PUBLIC_BASE_URL` | `http://localhost:3000` | 向后兼容/公共基础 URL（服务器运行时优先使用 `BASE_URL`） |
| `NEXT_PUBLIC_CLOUD_URL` | `https://toprouter.com` | 向后兼容/公共云 URL（服务器运行时优先使用 `CLOUD_URL`） |
...
- 使用历史和日志：`~/.toprouter/usage.json` 和 `~/.toprouter/log.txt`，由 `src/lib/usageDb.js` 管理。
...
- 验证 `CLOUD_URL` 指向您预期的云端端点（例如：`https://toprouter.com`）
- 尽可能保持 `NEXT_PUBLIC_*` 值与服务器端值一致。

**云端端点 `stream=false` 返回 500（`Unexpected token 'd'...`）**
- 症状通常出现在公共云端端点（`https://toprouter.com/v1`）的非流式调用上。
...
- `tester/security/test-cloud-openai-compatible.sh`
  - 使用提供的模型/密钥向云端端点（`https://toprouter.com/v1/chat/completions`）发送直接的 OpenAI 兼容请求。
...
- 公共云端端点（`https://toprouter.com/v1/chat/completions`）：
...
- **网站**：[toprouter.com](https://toprouter.com)
- **GitHub**：[github.com/decolua/toprouter](https://github.com/decolua/toprouter)
- **问题**：[github.com/decolua/toprouter/issues](https://github.com/decolua/toprouter/issues)
...
详细指南请参阅 [Pull Requests](https://github.com/decolua/toprouter/pulls)。
