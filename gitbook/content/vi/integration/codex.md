...
Tích hợp TopRouter với OpenAI Codex CLI để định tuyến request API OpenAI qua hệ thống routing thông minh của TopRouter.
...
TopRouter cung cấp các model Codex sau:
...
1. Xác minh API key đúng trong TopRouter dashboard
...
1. Xác minh tên model khớp với cấu hình TopRouter
2. Kiểm tra kết nối provider OpenAI đang hoạt động trong TopRouter dashboard
...
Đảm bảo bạn đã cấu hình API key trong TopRouter cloud dashboard.
