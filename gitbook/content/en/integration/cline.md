...
Integrate TopRouter with Cline VSCode extension to route your AI requests through TopRouter's intelligent routing system.

## Prerequisites

- Visual Studio Code installed
- Cline extension installed from VSCode marketplace
- TopRouter running locally or cloud endpoint configured
...
Set the base URL to your TopRouter endpoint:
...
**Steps:**
1. In the **Base URL** field, enter your TopRouter endpoint
...
3. The key should start with `sk-toprouter-`

### 5. Select Model

1. In the **Model** dropdown, you can either:
   - Select from available models (if Cline auto-detects them)
   - Manually enter the model name from your TopRouter configuration
...
4. Cline will use TopRouter to process your request

### Code Generation

1. Ask Cline to generate code: "Create a React component for a login form"
2. Cline will generate code using TopRouter
...
3. Get AI-powered explanations through TopRouter

### File Operations

1. Ask Cline to create, modify, or delete files
2. Cline will use TopRouter to understand context and make changes
...
1. Verify TopRouter is running: `curl http://localhost:20128/health`
...
2. Verify your TopRouter instance is running and healthy
3. Try reloading VSCode window (Cmd/Ctrl + Shift + P → "Reload Window")
4. Check TopRouter logs for any errors

## Advanced Configuration

### Using Cloud Endpoint

To use TopRouter cloud endpoint instead of localhost:

1. In Cline settings, set Base URL to: `https://toprouter.com`
2. Make sure you have configured your API key in the TopRouter cloud dashboard
...
## Integration with TopRouter Features

### Model Routing

TopRouter automatically routes your requests to the best available provider based on:
- Model availability
...
If a provider fails, TopRouter automatically falls back to alternative providers configured in your dashboard.

### Usage Tracking

Monitor your Cline usage through TopRouter dashboard:
- Total requests
- Token usage
- Cost per model
