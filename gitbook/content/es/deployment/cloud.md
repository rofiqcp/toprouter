...
Despliega TopRouter en VPS o Docker para acceso remoto y uso en producción.
...
| `DATA_DIR` | `~/.toprouter` | Ruta de almacenamiento de la base de datos |
...
```bash
sudo mkdir -p /var/lib/toprouter
sudo chown $USER:$USER /var/lib/toprouter
```
...
Crea `/etc/nginx/sites-available/toprouter`:
...
sudo ln -s /etc/nginx/sites-available/toprouter /etc/nginx/sites-enabled/
...
# Si NO usas proxy reverso, permite los puertos de TopRouter
...
tar -czf toprouter-backup-$(date +%Y%m%d).tar.gz /var/lib/toprouter

# Respaldo automatizado diario (agregar a crontab)
0 2 * * * tar -czf /backups/toprouter-$(date +\%Y\%m\%d).tar.gz /var/lib/toprouter
```
...
```bash
# Verificar si TopRouter está corriendo
pm2 status
...
sudo chown -R $USER:$USER /var/lib/toprouter
chmod 755 /var/lib/toprouter
```
