...
**TopRouterは、サブスクリプションの価値を最大化し、コストを最小限に抑えるAIモデルルーターです。**
...
**はい、TopRouter自体は100%無料でオープンソースです。**
...
**TopRouterはすべてのプロバイダーのクォータをリアルタイムで追跡します:**
...
## TopRouterをセルフホストできますか?

**はい! TopRouterは複数のデプロイメントオプションをサポートします:**
...
- `DATA_DIR` - データベース保存パス(デフォルト: `~/.toprouter`)
...
**はい、TopRouterはセキュリティとプライバシーを優先します:**

**ローカルストレージ:**
- すべてのデータは`~/.toprouter`(またはカスタム`DATA_DIR`)にローカル保存
- TopRouterサーバーへのデータ送信なし
- OAuthトークンはJWTで暗号化
...
**破壊的変更:**
- [CHANGELOG.md](https://github.com/decolua/toprouter/blob/main/CHANGELOG.md)を確認
- メジャー更新前に`~/.toprouter`をバックアップ
- メジャーバージョンの移行ガイドを確認
...
詳細は[CONTRIBUTING.md](https://github.com/decolua/toprouter/blob/main/CONTRIBUTING.md)を参照。

---

## さらにヘルプが必要?

- **ドキュメント:** [toprouter.com/docs](https://toprouter.com/docs)
