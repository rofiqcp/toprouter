# TopRouter Architecture

_Last updated: 2026-02-06_

## Executive Summary

TopRouter is a local AI routing gateway and dashboard built on Next.js.
...
flowchart LR
    ...
    subgraph Router[TopRouter Local Process]
...
- `src/lib/localDb.js`
- file: `${DATA_DIR}/db.json` (or `~/.toprouter/db.json` when `DATA_DIR` is unset)
- entities: providerConnections, providerNodes, modelAliases, combos, apiKeys, settings, pricing

Usage DB:

- `src/lib/usageDb.js`
- files: `~/.toprouter/usage.json`, `~/.toprouter/log.txt`
...
- main state: `${DATA_DIR}/db.json` (or `~/.toprouter/db.json`)
- usage stats: `~/.toprouter/usage.json`
- request log lines: `~/.toprouter/log.txt`
...
flowchart LR
    ...
    subgraph ContainerOrProcess[TopRouter Runtime]
        Next[Next.js Server\nPORT=20128]
...
1. `usageDb` currently stores under `~/.toprouter` and does not follow `DATA_DIR`.
...
- Build from source: `cd /root/dev/toprouter && npm run build`
- Build Docker image: `cd /root/dev/toprouter && docker build -t toprouter .`
