...
// Collect PIDs of all toprouter-related processes (excluding current)
  ...
  if (platform === "win32") {
    try {
      ...
      lines.forEach(line => {
        ...
        // Match anything running from toprouter install dir or wrapper cli.js
        const isAppProcess = lower.includes("toprouter") ||
    ...
    try {
      ...
      output.split("\n").forEach(line => {
        const isAppProcess = line.includes("toprouter") ||
...
function getDataDir() {
  ...
  if (process.platform === "win32") {
    return path.join(process.env.APPDATA || path.join(os.homedir(), "AppData", "Roaming"), "toprouter");
  }
  return path.join(os.homedir(), ".toprouter");
}
...
// Resolve npx/toprouter binary to relaunch after update (cross-platform)
function resolveRelaunchCommand() {
  const isWin = process.platform === "win32";
  // Prefer `npx toprouter` — works regardless of global bin path changes after npm i -g
