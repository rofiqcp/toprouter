...
export async function importDb(payload) {
  ...
  await db.transaction(async (tx) => {
    ...
    for (const [tool, mappings] of Object.entries(payload.mitmAlias || {})) {
      await tx.run(`INSERT OR REPLACE INTO kv(scope, key, value) VALUES('mitmAlias', ?, ?)`, [tool, stringifyJson(mappings || {})]);
    }
    for (const [provider, models] of Object.entries(payload.pricing || {})) {
      await tx.run(`INSERT OR REPLACE INTO kv(scope, key, value) VALUES('pricing', ?, ?)`, [provider, stringifyJson(models || {})]);
    }
  });
