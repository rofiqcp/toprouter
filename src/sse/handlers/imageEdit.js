// Image Edit handler — wraps imageGeneration but injects _kind: "imageEdit"
// so the provider adapter can select the correct URL.
...
export async function handleImageEdit(request) {
  ...
  // Inject kind so adapter routes to the correct endpoint
  body._kind = "imageEdit";
  ...
  if (comboModels) {
    ...
    log.info("IMAGE_EDIT", `Combo "${modelStr}" with ${comboModels.length} models (strategy: ${comboStrategy}, sticky: ${comboStickyLimit})`);
...
async function handleSingleModel(body, modelStr, { wantsStream, binaryOutput, preferredConnectionId } = {}) {
  ...
  if (NO_AUTH_PROVIDERS.has(provider)) {
    ...
    return errorResponse(result.status || HTTP_STATUS.BAD_GATEWAY, result.error || "Image edit failed");
  }
