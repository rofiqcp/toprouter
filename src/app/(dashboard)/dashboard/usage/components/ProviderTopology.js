...
function getProviderImageUrl(providerId) {
  // Custom connections use generated UUID-based ids (e.g.
  // "openai-compatible-chat-<uuid>", "custom-embedding-<uuid>") that have no
  // icon asset — return null so the node renders its textIcon fallback instead
  // of firing a guaranteed 404 request.
  if (providerId && /[0-9a-f]{8}-[0-9a-f]{4}/.test(providerId)) return null;
...
// Center TopRouter node — pulse/glow on card only (no expanding rings)
