...
async function buildCodexHeader() {
  ...
  const { installed, hasTopRouter, config } = result.data;
...
async function buildDroidHeader() {
  ...
  const { installed, hasTopRouter, settings } = result.data;
  ...
  // Extract TopRouter custom model config
  const custom = settings?.customModels?.find(m => m.id === "custom:TopRouter-0");
  const lines = [`Status:   ${COLORS.green}✓ Configured${COLORS.reset}`];
...
async function buildOpenClawHeader() {
  ...
  // Extract TopRouter provider config
...
async function buildOpenCodeHeader() {
  ...
  const { installed, hasTopRouter, opencode } = result.data;
...
async function showCliToolsMenu(port, breadcrumb = []) {
  ...
  await showMenuWithBack({
    title: "🔧 CLI Tools",
    breadcrumb,
    headerContent: `Configure CLI tools to use TopRouter\nEndpoint: ${endpoint}`,
    items: [
      {
        label: "Claude Code",
